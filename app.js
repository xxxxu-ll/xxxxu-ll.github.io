(() => {
  ...
  function select(a,q=0){selected=a;quote=q;libraryAuthor=a;page=0;renderReading();renderButtons();if(list)renderLibrary();draw();}
  ...
  function motionLabel(){document.body.classList.toggle('motion-paused',paused);lastTick=0;if(paused&&frame){cancelAnimationFrame(frame);frame=0;}$('motion').textContent=paused?'播放动画':'暂停动画';$('motion').setAttribute('aria-pressed',String(paused));$('motion').setAttribute('aria-label',paused?'播放星河动画':'暂停星河动画');}
  ...
  function setZoom(z){zoom=Math.min(1.3,Math.max(.75,z));$('zoom-reset').textContent=Math.round(zoom*100)+'%';$('zoom-out').disabled=zoom<=.75;$('zoom-in').disabled=zoom>=1.3;layout();draw();}
  ...
  const bridges=Array.from({length:420},(_,i)=>({a:i%meta.length,b:(i%meta.length+1+Math.floor(i/meta.length)%4)%meta.length,phase:rand(),speed:.25+rand()*.8,size:.3+rand()*.8}));
  function draw(){const t=sceneTime;if(!ctx||list)return;ctx.clearRect(0,0,width,height);field.forEach(s=>{ctx.fillStyle='rgba(194,208,234,'+(s.alpha*(.78+.22*Math.sin(t*.0003+s.phase)))+')';ctx.beginPath();ctx.arc(s.x*width,s.y*height,s.r,0,Math.PI*2);ctx.fill();});
    ctx.globalCompositeOperation='lighter';
    bridges.forEach(s=>{const pa=position(meta[s.a]),pb=position(meta[s.b]);const ax=pa.x*width,bx=pb.x*width,ay=pa.y*height-34,by=pb.y*height-34;const progress=(t*.000035*s.speed+s.phase)%1;const arc=Math.sin(progress*Math.PI)*Math.sin(s.phase*17)*26;const px=ax+(bx-ax)*progress+arc;const py=ay+(by-ay)*progress-arc*.55;ctx.globalAlpha=(.14+.35*Math.sin(progress*Math.PI))*((s.a===selected||s.b===selected)?1:.6);ctx.fillStyle=meta[(progress<.5?s.a:s.b)].color;ctx.beginPath();ctx.arc(px,py,s.size*(.8+.5*Math.sin(progress*Math.PI)),0,Math.PI*2);ctx.fill();});
    ctx.globalAlpha=1;
  ...
  function tick(t){frame=0;if(paused||document.hidden||list){lastTick=0;return;}if(lastTick)sceneTime+=Math.min(t-lastTick,50);lastTick=t;draw();frame=requestAnimationFrame(tick);}
  function start(){lastTick=0;if(!frame&&!paused&&!document.hidden&&!list)frame=requestAnimationFrame(tick);}
  function resize(){const rect=canvas.getBoundingClientRect();if(!rect.width)return;width=rect.width;height=rect.height;const dpr=Math.min(window.devicePixelRatio||1,2);canvas.width=width*dpr;canvas.height=height*dpr;if(ctx)ctx.setTransform(dpr,0,0,dpr,0,0);layout();draw();start();}
