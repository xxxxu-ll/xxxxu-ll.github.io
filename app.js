(() => {
  ...
  meta.forEach((m,a)=>{
    ...
    data[a].quotes.slice(0,starSlots).forEach((q,i)=>{const s=el('button',undefined,'quote-star');s.style.setProperty('--star-color',m.color);s.dataset.a=a;s.dataset.q=i;s.dataset.slot=i;s.setAttribute('aria-label',m.name+'：'+q.text);s.title=q.text;s.addEventListener('click',()=>selectStar(a,Number(s.dataset.q)));$('star-controls').append(s);});
  ...
  function quotePosition(a,i){const p=position(meta[a]),angle=i*2.39996+a*.6;const r=(32+i*7)*zoom;return{x:p.x*width+Math.cos(angle)*r,y:p.y*height+Math.sin(angle)*r*.66-38};}
