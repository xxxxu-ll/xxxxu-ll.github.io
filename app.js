(() => {
  ...
  meta.forEach((m,a)=>{
    ...
    data[a].quotes.slice(0,starSlots).forEach((q,i)=>{const s=el('button',undefined,'quote-star');s.style.setProperty('--star-color',m.color);s.dataset.a=a;s.dataset.q=i;s.dataset.slot=i;s.setAttribute('aria-label',m.name+'，第'+(i+1)+'则：'+q.text);s.title=q.text;s.addEventListener('click',()=>selectStar(a,Number(s.dataset.q)));$('star-controls').append(s);});
  ...
  function quotePosition(a,i){const p=position(meta[a]),count=data[a].quotes.length||starSlots,angle=i*2.39996+a*.6;const clusterRadius=Math.min(190,Math.max(95,Math.min(width*.2,height*.3)))*zoom;const r=clusterRadius*(.12+.88*Math.sqrt((i+1)/count));return{x:p.x*width+Math.cos(angle)*r,y:p.y*height+Math.sin(angle)*r*.62-38};}
  ...
  const clusters=meta.map(()=>Array.from({length:145},()=>{const a=rand()*Math.PI*2,r=Math.pow(rand(),1.7)*110;return{x:Math.cos(a)*r,y:Math.sin(a)*r*.55,r:.3+rand()*.8,alpha:.15+rand()*.7};}));
  function draw(t){if(!ctx||list)return;ctx.clearRect(0,0,width,height);field.forEach(s=>{ctx.fillStyle='rgba(194,208,234,'+(s.alpha*(paused?1:.78+.22*Math.sin(t*.0003+s.phase)))+')';ctx.beginPath();ctx.arc(s.x*width,s.y*height,s.r,0,Math.PI*2);ctx.fill();});
    meta.forEach((m,a)=>{const p=position(m),x=p.x*width,y=p.y*height-34,rad=(a===selected?155:115)*zoom;const glow=ctx.createRadialGradient(x,y,0,x,y,rad);glow.addColorStop(0,m.color+(a===selected?'1c':'10'));glow.addColorStop(.5,m.color+'06');glow.addColorStop(1,m.color+'00');ctx.fillStyle=glow;ctx.fillRect(x-rad,y-rad,rad*2,rad*2);clusters[a].forEach(s=>{ctx.globalAlpha=s.alpha*(a===selected?1:.65);ctx.fillStyle=m.color;ctx.beginPath();ctx.arc(x+s.x*zoom,y+s.y*zoom,s.r,0,Math.PI*2);ctx.fill();});ctx.globalAlpha=1;
